...
QNetworkReply* Transmission::rpcMethod(const QString &name, QJsonObject arguments)
{
    ...
    if (!user.isEmpty()) {
        ...
        request.setRawHeader("Authentication", QString("Basic %1").arg(QString(cred.toLatin1().toBase64())).toLatin1());
    }
        ...
        connect(reply, &QNetworkReply::finished, [this, reply]() {
            ...
            if (document.isNull()) {
                ...
                if (response.value("arguments").toObject().contains("torrent-duplicate"))
                    qCritical()<< "torrent duplicate"<< response.value("arguments").toObject().value("torrent-duplicate").toObject().value("name").toString();
                else if (response.value("arguments").toObject().contains("torrent-added"))
                    qCritical()<< "torrent added"<< response.value("arguments").toObject().value("torrent-added").toObject().value("name").toString();
            }
        });
